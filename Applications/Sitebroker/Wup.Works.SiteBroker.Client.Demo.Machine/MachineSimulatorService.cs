...
internal sealed class MachineSimulatorService : BackgroundService
{
    ...
    private readonly ITelemetryProducerService _telemetry;
    ...
    public MachineSimulatorService(
        ...
        ITelemetryProducerService telemetry,
    ...
    private async Task PublishDemoData()
    {
        ConsoleUi.Write(
            ...
            "  [4] Parts +1             [5] Cycles +1           [6] Meter +12\n" +
        ...
        {
            case "1":
                await _telemetry.PublishMachineState(MachineState.Working);
                ConsoleUi.Success("-> state = Working (3)");
                break;
            case "2":
                await _telemetry.PublishMachineState(MachineState.Idle);
                ConsoleUi.Success("-> state = Idle (2)");
                break;
            case "3":
                await _telemetry.PublishMachineState(MachineState.Error);
                ConsoleUi.Success("-> state = Error (4)");
                break;
            case "4":
                await _telemetry.PublishMachineParts(++_parts);
                ConsoleUi.Success($"-> parts = {_parts}");
                break;
            case "5":
                await _telemetry.PublishMachineCycles(++_cycles);
                ConsoleUi.Success($"-> cycles = {_cycles}");
                break;
            case "6":
                _meter += 12;
                await _telemetry.PublishMachineMeters(_meter);
                ConsoleUi.Success($"-> meter = {_meter}");
                break;
            case "7":
                await _telemetry.PublishMachineProgram("P-4711");
                ConsoleUi.Success("-> program = P-4711");
                break;
            case "8":
                await _telemetry.PublishErrorDescription("E-1042", "Vacuum too low");
                ConsoleUi.Success("-> error[E-1042] = 'Vacuum too low'");
                break;
            case "9":
                await _telemetry.RemoveErrorDescription("E-1042");
                ConsoleUi.Success("-> error[E-1042] cleared (empty retained payload)");
                break;
            case "s":
                await _telemetry.PublishStorageLevel("S1", 42);
                await _telemetry.PublishStorageCapacity("S1", 100);
                await _telemetry.PublishStorageMaterial("S1", "PUR-Glue");
    ...
    private void PrintMenu()
    {
        ConsoleUi.Write(
            ...
            "  [l]     Show open jobs\n" +
            "  [d]     Publish machine telemetry (state / counters / error / storage)\n" +
