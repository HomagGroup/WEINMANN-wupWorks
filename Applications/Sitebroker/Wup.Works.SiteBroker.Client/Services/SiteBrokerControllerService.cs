...
public class SiteBrokerControllerService : ISiteBrokerControllerService, IDisposable
{
    ...
    private readonly string _orderLoadedTopic = OrderTopicHelper.GetOrderLoadedTopic(Constants.Wildcard, Constants.Orchestrator, Constants.Wildcard);
    private readonly string _batchPreparedTopic = OrderTopicHelper.GetBatchPreparedTopic(Constants.Wildcard, Constants.Orchestrator, Constants.Wildcard);
    private readonly string _batchVariantProducedTopic = OrderTopicHelper.GetBatchVariantProducedTopic(Constants.Wildcard, Constants.Orchestrator, Constants.Wildcard);
    private readonly string _onlineModeChangedTopic = OrderTopicHelper.GetOnlineModeTopic(Constants.Wildcard, Constants.Orchestrator);
    ...
    public async Task SendLoadOrderRequest(string machineNumber, Guid orderId, string? fileName)
    {
        ...
        var loadOrderTopic = OrderTopicHelper.GetOrderLoadTopic(machineNumber, Constants.Orchestrator, orderId.ToString());
    ...
    public async Task SendPrepareBatchRequest(string machineNumber, Guid batchId, Guid? orderId, string? variant)
    {
        ...
        if (orderId != null)
            payload.AdditionalProperties.Add(Constants.OrderId, orderId.Value.ToString());

        if (variant != null)
            payload.AdditionalProperties.Add(Constants.Variant, variant);

        var prepareBatchTopic = OrderTopicHelper.GetBatchPrepareTopic(machineNumber, Constants.Orchestrator, batchId.ToString());
    ...
    public async Task SendExecuteBatchVariantRequest(string machineNumber, Guid batchVariantId, Guid? batchId, Guid? orderId = null)
    {
        ...
        if (batchId != null)
            payload.AdditionalProperties.Add(Constants.BatchId, batchId.Value.ToString());

        if (orderId != null)
            payload.AdditionalProperties.Add(Constants.OrderId, orderId.Value.ToString());

        var produceBatchVariantTopic = OrderTopicHelper.GetBatchVariantProduceTopic(machineNumber, Constants.Orchestrator, batchVariantId.ToString());
    ...
    {
        var loadOrderTopic = OrderTopicHelper.GetOrderLoadedTopic(machineNumber, Constants.Orchestrator, orderId.ToString());
    ...
    {
        var prepareBatchTopic = OrderTopicHelper.GetBatchPreparedTopic(machineNumber, Constants.Orchestrator, batchId.ToString());
    ...
    {
        var produceBatchVariantTopic = OrderTopicHelper.GetBatchVariantProducedTopic(machineNumber, Constants.Orchestrator, batchVariantId.ToString());
    ...
    private static IDictionary<string, string> GetAdditionalProperties(string payload)
    {
        var content = GetPayload(payload);
        return content?.AdditionalProperties ?? new Dictionary<string, string>();
    }

    private static GenericPayloadDto GetPayload(string payload)
    {
