...
public class MqttClientService : IMqttClientService
{
    ...
    private static readonly Collection<string> _subscribedTopics = new();
    ...
    public async Task Connect()
    {
        ...
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            ...
            _logger.LogDebug(_options.CleanSession ? "[MQTT] Client {id} using a clean session" : "[MQTT] Client {id} preserves the session",
                _options.Id);
        ...
        mqttEventLogger.LogMessagePublished += (_, args) =>
        {
            ...
            switch (args.LogMessage.Level)
            {
                ...
                case MqttNetLogLevel.Warning:
                    _logger.LogWarning(output.ToString());
                    break;
    ...
    private async Task SubscribeIfConnected(string topic, int qos)
    {
        ...
        await _managedMqttClient.SubscribeAsync(new List<MqttTopicFilter>
        {
