...
/// <summary>
/// Builds and matches the command/order topics of the SiteBroker command interface
/// (order load, batch prepare, batch-variant produce, online mode).
/// </summary>
public static class OrderTopicHelper
{
    /// <summary>
    ...
    public static bool ValidateTopic(this string subscribedTopic, string receivedTopic)
    {
        ...
        if (subscribedTopicParts.Length != receivedTopicParts.Length)
            return false;

        for (int i = 0; i < subscribedTopicParts.Length; i++)
        {
